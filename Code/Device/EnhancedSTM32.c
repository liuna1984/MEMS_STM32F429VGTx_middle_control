...
extern XL_UART_IT_InputStreamTypeDef g_Usart2InputStream;
/* Private typedef -----------------------------------------------------------*/
/* Private macro -------------------------------------------------------------*/
/** @addtogroup LED
  * @{
...
void SendDoubleBuffer(DoubleBuffer *pBuf, UART_HandleTypeDef* huart)
{
	if (huart->gState != HAL_UART_STATE_BUSY_TX && pBuf->m_nBufferLength[pBuf->m_nBufferNumber] > 0)
	{
		...
		{
			// RS485 TX
			 HAL_GPIO_WritePin(RD_485_1_GPIO_Port, RD_485_1_Pin, GPIO_PIN_SET);
	//		 HAL_Delay(10);
		}
		{
			HAL_UART_Transmit_DMA(huart, pBuf->m_ucDataBuffer[1 - pBuf->m_nBufferNumber],pBuf->m_nBufferLength[1 - pBuf->m_nBufferNumber]);
	//		HAL_UART_Transmit(huart, pBuf->m_ucDataBuffer[1 - pBuf->m_nBufferNumber],pBuf->m_nBufferLength[1 - pBuf->m_nBufferNumber],100);
		}
//		  if(huart->Instance == USART2)
//		{
//			// RS485 RX
...
//		  HAL_UART_Receive_IT(g_Usart2InputStream.huart, &(g_Usart2InputStream.ucTemp), 1);
//		  
//		}
