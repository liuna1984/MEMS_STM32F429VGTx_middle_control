...
extern uint8_t SendOverFlag;
extern uint32_t RecCount;
extern uint8_t f125ms;
...
//	static unsigned short usFrameCounter = 0;
	...
	case 0x01://返回IMU数据（usart2接收，通过usart1发送）
...
//	    QSENS_MakeImuFrame1(&g_Imu, &g_Usart3OutputBuffer,CommandAddr);
//	    SendDoubleBuffer(g_Usart3OutputStream.doublebuffer, &huart3);
//		g_Imu.receive.Status = MESSAGE_STATUS_AVALABEL;
	    QSENS_MakeImuFrame2(&pBuffer[8],36,&g_Usart1OutputBuffer,CommandAddr,1);
	    SendDoubleBuffer(g_Usart1OutputStream.doublebuffer, &huart1);;
		break;
	case 0x02://返回写入OK（usart2接收，通过usart1发送）
		
		QSENS_MakeImuFrame2(&pBuffer[8],1,&g_Usart1OutputBuffer,CommandAddr,1);
	    SendDoubleBuffer(g_Usart1OutputStream.doublebuffer, &huart1);
		break;
	case 0x03://返回标定参数（usart2接收，通过usart1发送）
		QSENS_MakeImuFrame2(&pBuffer[8],188,&g_Usart1OutputBuffer,CommandAddr,1);
	    SendDoubleBuffer(g_Usart1OutputStream.doublebuffer, &huart1);
		break;
	case 0x04://写入标定参数（usart1接收，通过usart2发送）
		QSENS_MakeImuFrame2(&pBuffer[8],188,&g_Usart2OutputBuffer,CommandAddr,2);
	    SendDoubleBuffer(g_Usart2OutputStream.doublebuffer, &huart2);
		break;
	case 0x05://读标定参数（usart1接收，通过usart2发送）
		QSENS_MakeImuFrame2(&pBuffer[8],1,&g_Usart2OutputBuffer,CommandAddr,2);
	    SendDoubleBuffer(g_Usart2OutputStream.doublebuffer, &huart2);
		break;
	case 0x07://测量模式（usart1接收，通过usart1发送）
		SendFlag=1;
	    QSENS_MakeImuFrame2(&pBuffer[8],1,&g_Usart1OutputBuffer,CommandAddr,1);
	    SendDoubleBuffer(g_Usart1OutputStream.doublebuffer, &huart1);
		break;
	case 0x08://配置模式（usart1接收，通过usart1发送）
		SendFlag=0;
	    QSENS_MakeImuFrame2(&pBuffer[8],1,&g_Usart1OutputBuffer,CommandAddr,1);
	    SendDoubleBuffer(g_Usart1OutputStream.doublebuffer, &huart1);
		break;
	case 0x09://发送完成标识（usar2接收）
		SendOverFlag=1;
	    RecCount=0;
//	    f125ms=0;
//	    QSENS_MakeImuFrame2(&pBuffer[8],1,&g_Usart1OutputBuffer,CommandAddr,1);
...
void QSENS_MakeImuFrame2(uint8_t *pBuffer,uint16_t len,DoubleBuffer *buf,uint16_t CommandAddr,uint8_t usFrameFlag)
{
	static unsigned short usFrameCounter = 0;
	static unsigned short usFrameCounter1[8]={0};
	static unsigned short usFrameCounter2 = 0;
	uint8_t Addr=0;
	int i;
	Addr=(uint8_t)(CommandAddr&0xFF);
	FrameBegin(buf, &frameConstr);
	PushDoubleBuffer(0xEA, buf);
	PushDoubleBuffer(0x51, buf);
	CheckSectionBegin(buf, &frameConstr);
...
//	usFrameCounter++;
	if(usFrameFlag==1)
	{
		PUSH_VAR_DOUBLEBUFFER(usFrameCounter1[Addr], buf);
		usFrameCounter1[Addr]++;		
	}
	else if(usFrameFlag==2)
	{
		PUSH_VAR_DOUBLEBUFFER(usFrameCounter2, buf);
		usFrameCounter2++;		
	}
	for(i=0;i<len;i++)
	{
